					<div class="homeClass">
						<div class="container-fluid">
							...
							<form class="" action="<?= base_url()?>Stock/apci/sell-utility" method="post">
								<div class="row">
								  <div class="Class-4">
										<div class="form-group">
										  <label for="Proveedor"> Proveedor </label>
										  <select class="form-control" id="Proveedor" name="Proveedor">
										    <option value="selecciona-el-proveedor"> Selecciona el cliente </option>
												<?php foreach ($empleado as $row){ ?>
													<option value="<?= $row -> idEmp ?>"> <?= $row -> ApellidoPEmp.' '.$row -> ApellidoMEmp.' '.$row -> NomEmp?> </option>
												<?php } ?>
										  </select>
										</div>
								  </div>
