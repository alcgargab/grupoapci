							...
							<?php if ($groupby != ""){ ?>
											...
											<?php
												...
												for ($i = 0; $i <= $total -1 ; $i++) {
													foreach ($ejecutivo as $row) {
														if ($row -> IdUsuario == $groupby[$i] -> C_Id_U) {
															echo "['". $row -> CallNombre ."', ".$groupby[$i] -> NumContador."],";
														}
													}
												}
											?>
								...
								<form class="" action="<?= base_url()?>Supervisor/reportes/<?= $reporte?>" method="post">
									...
									<div class="row">
										<div class="col-lg-3">
											<br>
										  <input type="submit" class="btn btn-outline-primary btn-block" name="" value="Total de <?= $totalRegistros ?> Registros">
										</div>
										<!-- <div class="col-lg-1">
											<br>
											<button class="btn btn-outline-success btn-block" onclick="changeOptions();"> 3D </button>
										</div> -->
										<div class="col-5">
